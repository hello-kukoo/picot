...
/// SKILL.md frontmatter is parsed by the same parser the discovery surface
/// uses (`host_skills::parse_frontmatter`): one contract for both scans,
/// including YAML block-scalar descriptions (`>` folded, `|` literal) that
/// common SKILL.md files use.
fn parse_frontmatter(content: &str) -> ParsedFrontmatter {
    let (name, description, disable_model_invocation) =
        crate::host_skills::parse_frontmatter(content);
    ParsedFrontmatter {
        name,
        description,
...
pub fn scan_install_source(
    ...
    fn find_group_mut<'a>(
        nodes: &'a mut [SkillTreeNode],
        id: &str,
    ) -> Option<&'a mut SkillInstallGroup> {
        for node in nodes {
            let SkillTreeNode::Group(group) = node else {
                continue;
            };
            if group.id == id {
                return Some(group);
            }
            if let Some(found) = find_group_mut(&mut group.children, id) {
                return Some(found);
            }
        }
        None
    ...
    fn ensure_group(
        ...
        if path_parts.len() == 1 {
            ...
            if let Some(parent_node) = find_group_mut(tree, &parent.id) {
    ...
    for skill in &sorted_skills {
        ...
        if parts.is_empty() {
            ...
            if let Some(group_node) = find_group_mut(&mut tree, &_group.id) {
                group_node
                    .children
...
mod tests {
    ...
    fn binding(source_dir: &Path, secret: &str) -> (SkillSourceBinding, InstallContext) {
        let binding = SkillSourceBinding {
            ...
            owner_id: OwnerId::from_string("client-1".to_string()),
    ...
    #[test]
    fn scan_reads_folded_and_literal_description_blocks() {
        let tmp = tempdir().unwrap();
        let folded = tmp.path().join("folded");
        let literal = tmp.path().join("literal");
        ...
        fn collect_descriptions<'a>(
            ...
            descriptions: &mut Vec<(&'a str, &'a str)>,
        ) {
            for node in nodes {
                match node {
                    SkillTreeNode::Candidate(candidate) => {
                        descriptions.push((&candidate.name, &candidate.description));
                    }
                    SkillTreeNode::Group(group) => {
                        collect_descriptions(&group.children, descriptions)
                    }
                }
            }
        }
        let mut descriptions = Vec::new();
        collect_descriptions(&scan.tree, &mut descriptions);
        assert!(scan.diagnostics.is_empty(), "{:?}", scan.diagnostics);
        assert!(descriptions.contains(&("folded", "first folded line second folded line")));
        assert!(descriptions.contains(&("literal", "first literal line\nsecond literal line")));
    }
    ...
    #[test]
    fn scan_keeps_three_level_groups_nested() {
        let tmp = tempdir().unwrap();
        write_skill(
            &tmp.path().join("group-a/group-b/group-c"),
            "deep-skill",
            "Nested three levels",
        );
        let (binding, context) = binding(tmp.path(), "secret");
        let scan = scan_install_source(&binding, &context);
        assert_eq!(scan.tree.len(), 1, "{:?}", scan.tree);
        let SkillTreeNode::Group(group_a) = &scan.tree[0] else {
            panic!("expected group-a");
        };
        let SkillTreeNode::Group(group_b) = &group_a.children[0] else {
            panic!("expected group-b");
        };
        let SkillTreeNode::Group(group_c) = &group_b.children[0] else {
            panic!("expected group-c");
        };
        assert!(matches!(group_c.children[0], SkillTreeNode::Candidate(_)));
    }
    ...
    fn install_writes_new_skill_entry_atomically() {
        ...
        let binding = SkillSourceBinding {
            ...
            canonical_path: source.clone(),
            owner_id: OwnerId::from_string("client-2".to_string()),
    ...
    fn install_rejects_stale_revision() {
        ...
        let binding = SkillSourceBinding {
            ...
            owner_id: OwnerId::from_string("client-3".to_string()),
