...
// Seed a minimal en message tree so t() resolves git.* keys instead of
// returning the raw key, making assertions on rendered text meaningful.
setMessages({
  git: {
    panel: "Git",
    files: "Files",
...
describe("GitPanel", () => {
  ...
  it("renders directories and files as distinct tree rows", () => {
    ...
    // Directory rows render an expanded folder SVG (Material object glyph),
    // matching File Browser's shared icon vocabulary.
    const folderIcon = panel.container.querySelector(
      ".git-directory-heading .git-tree-folder-icon svg",
    );
