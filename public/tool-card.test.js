...
describe("ToolCardRenderer file references", () => {
  it("turns a path argument into a preview button that emits previewfile", async () => {
    ...
    const renderer = new ToolCardRenderer(container, { enableFileRefs: true });
    renderer.createToolCard({
      toolCallId: "write-1",
      toolName: "write",
      status: "complete",
  ...
  it("suppresses the file-ref button unless file references are enabled", async () => {
    vi.stubGlobal("fetch", makeFetchMock());
    const { initI18n } = await importFreshI18n();
    await initI18n();
    const { ToolCardRenderer } = await import("./ui/tool-card.js");
    ...
    const container = document.createElement("div");
    const renderer = new ToolCardRenderer(container); // default: no previewfile listener
    renderer.createToolCard({
      toolCallId: "write-2",
      toolName: "write",
      status: "complete",
    ...
    // The path is still shown, but not as a focusable preview button.
    expect(container.querySelector(".tool-file-ref")).toBeNull();
    expect(container.querySelector(".tool-args-preview")?.textContent).toBe("docs/index.html");
  });
