...
Git History read commands (`log`, `log_detail`, `commit_diff`) remain owner-scoped native broker traffic. `log` follows the first-parent chain with host-clamped pagination; detail and diff validate a 40/64-hex commit OID and re-verify it with `rev-parse`. `commit_diff` authorizes its path by re-deriving that commit's own NUL-delimited name-status list, independently of status snapshots, then compares merge commits against their first parent and adds `--root` for root commits. All path-bearing commands use `--literal-pathspecs` and the existing bounded Git runner.
