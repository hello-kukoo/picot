...
const allowedInToml = [...toml.matchAll(/commands\.allow\s*=\s*\[([^\]]+)\]/g)]
  .flatMap((m) =>
    m[1]
      .split(',')
...
if (capabilityPermissions.includes('default') || capabilityPermissions.includes('pi-desktop:default')) {
  pass(`capabilities/default.json includes the app default permission set`);
} else {
  fail(`capabilities/default.json does NOT include "default" — custom commands will be blocked at runtime`);
}
