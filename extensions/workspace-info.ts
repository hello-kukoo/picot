...
export function resolveWorkspaceInfoPath(
  ...
  // `workspace:<cwd>` ids are generated by this server's own withRouteMeta
  // (mirror_sync snapshots et al.). Resolve them with the same running-instance
  // guard as path: so broadcast ids can be re-queried, never spoofed.
  if (workspaceId.startsWith("workspace:")) {
    const requested = workspaceId.slice("workspace:".length);
    return instances.find((i) => normalizePath(i.cwd) === normalizePath(requested))?.cwd ?? null;
  }
  return null;
}
