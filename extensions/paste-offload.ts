...
// ABOUTME: Keeps temporary prompt files contained, self-ignored, and addressable by @-file references.
...
export type PasteOffloadResult = {
  absolutePath: string;
  relativePath: string;
};
...
function ensureContainedDirectory(workspaceRoot: string, directory: string): string {
  const realDirectory = fs.realpathSync(directory);
  if (!isPathWithinRoot(workspaceRoot, realDirectory)) {
    throw new Error("Paste directory is outside workspace");
  }
  if (!fs.statSync(realDirectory).isDirectory()) {
    throw new Error("Paste path is not a directory");
  }
...
export function writePasteOffloadFile(
  ...
  for (let attempt = 0; attempt < 100; attempt += 1) {
    ...
    const fileName = `${baseName}${suffix}${FILE_SUFFIX}`;
