...
| `native_launch_spec`（impl 方法，`pi_manager.rs:538`） | 当前 native debug 路径的 launch 解析入口（本文 §2.1/§3.1 依赖它） | 归入 P1 统一 `LaunchDescription`/native launch service（§13.1/§13.2）；删除需待 C-GAP-01–04 关闭后随 legacy manager 一并处置 |
