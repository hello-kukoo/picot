...
| Allow stores | v1: read-only counts (global + current project) with a hint that management happens in-session via the guard dialogs. Editing the allow list from a settings page invites rubber-stamping dangerous grants. |
