...
**修订 2026-09-22**：决策 4 的**落点**从全屏模态改为**内联进触发它的那个 turn**
（决策 4a）。卡片内容与按钮语义不变。
...
- Picot 富卡片组件 `public/ui/safety-guard-dialog.js`：
  `handleExtensionUIRequest(request)` 检测 `method==="select"` + message 含
  标记 → 解析 → 渲染卡片并应答；解析失败返回 false 走通用对话框。
  渲染落点由 `resolveHost(request)` 决定（**只在本组件已认领该请求后才调用**）：
  返回元素 → 内联渲染（`role="group"`，非模态）；返回 null → `container` 模态
  （`role="dialog"`）。`closeLiveTurn` 在 transcript 丢掉承载 turn 之前调用
  `rehost()`，把仍在等待的卡片**移动**进模态容器——卡片节点移动，状态与点击
  处理器随之存活；`rehost()` 从不代答（销毁卡片会让该 runtime 永远悬在
  `extension_ui_response` 上，代答 `cancelled` 等于静默 Block）。
  **Esc 只在卡片内生效**（监听挂在卡片节点、`stopPropagation()`）：内联卡片与
  输入框、停止按钮共处一页，页面级 Esc 若被卡片劫持成静默 Block 是语义事故；
  卡片出现时默认焦点已在卡内，Esc 仍然即时可用。
  接线点：主聊天 `app.js`（questionnaireCard 之后、switch 之前）；侧聊
  `ephemeral-chat-view.js` 的 select 渲染分支同样先过拦截（侧聊不传
  `resolveHost`，保持面板内联容器）。
